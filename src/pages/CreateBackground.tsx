      ...
      if (response.status === 'confirmed' && response.background.blockchainId) {
        ...
        toast.success(
          <div>
            ...
            <div className="mt-2">
              <span className="font-semibold">Blockchain ID:</span> {response.background.blockchainId}
            </div>
          </div>,
          { duration: 10000 }
        );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!authStatus.isAuthenticated) {
        authLog('Authentication check failed before minting', { message: authStatus.message }, 'error');
