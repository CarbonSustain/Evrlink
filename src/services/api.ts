...
if (!API_BASE_URL.startsWith('http')) {
  ...
  console.error('This will cause API requests to fail. Please check your VITE_API_URL environment variable.');
}
...
const getToken = () => {
  try {
    ...
    if (parts.length !== 3) {
      authLog('Token does not appear to be in JWT format', { token: token.substring(0, 15) + '...' }, 'warn');
    } else {
      authLog('Found valid JWT token format', { tokenPreview: token.substring(0, 15) + '...' });
    }
...
export const getAuthHeaders = () => {
  ...
  if (token) {
    ...
    apiLog('Added Authorization header to request', { headerPreview: `Bearer ${token.substring(0, 15)}...` });
  } else {
    apiLog('No auth token available - request may fail if authentication is required', null, 'warn');
  }
...
export const fetchBackgrounds = async (
  ...
  try {
    ...
    const response = await axios.get(`${API_BASE_URL}/api/background`, { params });
...
export const getBackgroundCategories = async (): Promise<string[]> => {
  try {
    const response = await axios.get(`${API_BASE_URL}/api/background/categories`);
...
export const mintBackgroundNFT = async (data: { 
  ...
  try {
    ...
    if (!walletAddress) {
      authLog('No wallet address found when attempting to mint NFT', null, 'error');
    ...
    const response = await axios.post(`${API_BASE_URL}/api/background/mint`, formData, {
      headers: {
        ...getAuthHeaders(),
        'Content-Type': 'multipart/form-data',
      },
    ...
    if (response.data && response.data.success) {
      ...
      apiLog('Background created and event emitted', { backgroundId: backgroundData.id });
    }
    ...
    if (error.response && error.response.status === 401) {
      authLog('Authentication error during minting - token may be invalid', {
...
export const verifyBackgroundStatus = async (backgroundId: string | number): Promise<any> => {
  try {
    const id = backgroundId.toString();
    const response = await axios.get(`${API_BASE_URL}/api/background/verify/${id}`, {
...
export const transferGiftCard = async (data: {
  ...
  try {
    const response = await axios.post(`${API_BASE_URL}/api/gift-cards/transfer`, {
...
export const setGiftCardSecret = async (data: {
  ...
  try {
    const response = await axios.post(`${API_BASE_URL}/api/gift-cards/${data.giftCardId}/set-secret`, {
      secret: data.secret
    }, {
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('token')}`
...
export const claimGiftCard = async (data: {
  ...
  try {
    const response = await axios.post(`${API_BASE_URL}/api/gift-cards/claim`, {
      giftCardId: data.giftCardId,
      secret: data.secret
    }, {
      headers: getAuthHeaders()
...
export const buyGiftCard = async (data: {
  ...
  try {
    const response = await axios.post(`${API_BASE_URL}/api/gift-cards/buy`, {
      giftCardId: data.giftCardId,
...
export const updateUserProfile = async (data: {
  ...
  try {
    ...
    const response = await axios.post(`${API_BASE_URL}/api/user/profile`, userData, {
      headers: getAuthHeaders(),
    });
...
export const getTopUsers = async (limit: number = 10): Promise<User[]> => {
  try {
    const response = await axios.get(`${API_BASE_URL}/api/user/top?limit=${limit}`, {
      headers: getAuthHeaders(),
    });
    return response.data.users || response.data;
  } catch (error) {
...
export const searchUsers = async (query: string): Promise<User[]> => {
  try {
    const response = await axios.get(`${API_BASE_URL}/api/user/search?query=${encodeURIComponent(query)}`, {
      headers: getAuthHeaders(),
    });
    return response.data.users || response.data;
  } catch (error) {
...
export const getUserActivity = async (walletAddress: string): Promise<any[]> => {
  try {
    const response = await axios.get(`${API_BASE_URL}/api/user/${walletAddress}/activity`, {
      headers: getAuthHeaders(),
    });
...
export const getRecentTransactions = async (limit: number = 10): Promise<Transaction[]> => {
  try {
    const response = await axios.get(`${API_BASE_URL}/api/user/transactions/recent?limit=${limit}`, {
      headers: getAuthHeaders(),
    });
...
export const loginWithWallet = async (walletAddress: string, signature: string): Promise<any> => {
  try {
    ...
    authLog('Attempting login with wallet', { 
      ...
      signatureType: signature.startsWith('mock_signature_for_') ? 'mock' : 'real'
    });
    ...
    throw new Error('Failed to login with wallet: ' + (error.message || 'Unknown error'));
  }
};
...
export const checkAuthState = async (): Promise<{ isAuthenticated: boolean, message: string }> => {
  try {
    const token = localStorage.getItem('token');
    const walletAddress = localStorage.getItem('walletAddress');
    ...
    if (!token || !walletAddress) {
      authLog('Not authenticated - missing token or wallet address', {
        hasToken: !!token,
      ...
      if (parts.length === 3) {
        ...
        if (payload.exp) {
          ...
          if (expiryDate.getTime() - now.getTime() < 10 * 60 * 1000) {
            authLog('Token is expired or about to expire', {
    ...
    try {
      ...
      if (response.data && (response.data.user || response.data.walletAddress)) {
        ...
        authLog('Authentication validation failed - invalid response', response.data, 'warn');
        return { 
          isAuthenticated: false, 
      ...
      if (error.response && error.response.status === 401) {
        ...
        try {
          ...
          // Create a mock signature for development
          const signature = `mock_signature_for_${walletAddress}`;
          const refreshResponse = await loginWithWallet(walletAddress, signature);
          
          if (refreshResponse && refreshResponse.token) {
            authLog('Authentication refreshed successfully after validation failure');
            return { 
              isAuthenticated: true, 
              message: 'Authentication refreshed successfully' 
            };
          ...
          authLog('Failed to refresh authentication after validation failure', refreshError, 'error');
        }
      }
      
      return { 
        isAuthenticated: false, 
        message: 'Authentication validation failed: ' + (error.message || 'Unknown error') 
      };
    }
  } catch (error: any) {
