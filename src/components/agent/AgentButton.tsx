...
export const AgentButton = () => {
  ...
  const [useEmbeddedChatbot, setUseEmbeddedChatbot] = useState(true);
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50">
      {/* Agent chat popup */}
      {isOpen && (
        <div className={`absolute bottom-16 right-0 mb-2 ${isMinimized ? 'w-[400px] h-[60px]' : 'w-[600px] h-[750px]'} bg-white rounded-lg shadow-xl overflow-hidden transition-all duration-300`}>
          {useEmbeddedChatbot ? (
            // Embedded chatbot iframe
            <div className="w-full h-full flex flex-col">
              <div className="bg-blue-600 text-white p-2 flex justify-between items-center">
                <h2 className="text-lg font-bold">Enhanced AgentKit Chatbot</h2>
                <div className="flex items-center space-x-2">
                  <button 
                    onClick={toggleChatbotMode}
                    className="text-xs bg-blue-700 hover:bg-blue-800 px-2 py-1 rounded"
                    title="Switch to local implementation"
                  >
                    Switch Mode
                  </button>
                  <button 
            ...
            <div className="w-full h-full flex flex-col">
              <div className="bg-blue-600 text-white p-2 flex justify-between items-center">
                <h2 className="text-lg font-bold">Local Agent Implementation</h2>
                <div className="flex items-center space-x-2">
                  <button 
                    onClick={toggleChatbotMode}
                    className="text-xs bg-blue-700 hover:bg-blue-800 px-2 py-1 rounded"
                    title="Switch to embedded chatbot"
                  >
                    Switch to Online
                  </button>
                  <button 
