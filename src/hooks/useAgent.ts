...
async function messageAgent(userMessage: string, offlineMode: boolean = false, userId: string = "default"): Promise<string | null> {
  ...
  try {
    // Try the enhanced AgentKit chatbot endpoint first (running on port 3002)
    const chatbotUrl = "http://localhost:3002/api/agent";
...
/**
 * This hook manages interactions with the AI agent.
 ...
 * - `isOfflineMode`: Whether the agent is in offline mode.
